...
<jsp:useBean id="utlBen" scope="page" class="mktg.backingBean.utlBean" />    
  ...
  <sql:query 
        sql="SELECT route.id,route.route, route.timing FROM route WHERE saletype = 1 " 
...
<html>
    ...
    <body>
        <form action="selectRoute_1.jsp" method="get">
            <h4>INDENT</h4>
                        ...
                        <select name="rt">
                            <c:forEach var="row" items="${rt.rows}">
                                <c:forEach var="column" items="${row.route}">
                                    <option value="${row.id}" > <c:out value ="${column}"/> </option>                                        \                                                               
                                </c:forEach>
                            </c:forEach>                              
                        </select>
                    </td>
